...
```
├── Core/          # 核心系统 (窗口、输入、数学)
├── Renderer/      # 渲染系统 (几何体、着色器)
├── Geometry/      # 几何体实现 (立方体、球体)
└── Main/          # 应用程序入口
```
...
**Geometry 抽象层**
- `IGeometry` 接口定义几何体标准接口
- `GeometryFactory` 工厂模式实现几何体注册和创建
...
**几何体实现**
- `Cube` 类：立方体几何体，采用循环编码优化
- `Sphere` 类：球体几何体，支持可配置的经纬度分段
